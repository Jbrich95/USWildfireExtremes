...
locBranch <- input_linear %>%
  layer_dense(
    units = 1,
    activation = 'linear',
    input_shape = dim(X)[2:4],
    name = 'lin_loc',
    weights = list(matrix(0, nrow = dim(X)[4], ncol = 1), array(log(init_loc)))
  )
...
spreadBranch <- input_linear %>%
  layer_dense(
    units = 1,
    activation = 'linear',
    input_shape = dim(X)[2:4],
    name = 'lin_s',
    weights = list(matrix(0, nrow = dim(X)[4], ncol = 1), array(log(init_spread)))
  )
...
model <- keras_model(
  inputs = c(input_linear, input_u, input_coords),
  outputs = c(output)
)
...
checkpoint <- callback_model_checkpoint(
  paste0("intermediates/models/bGEV_linear_fit/boot_", boot.num),
  monitor = "val_loss",
  verbose = 0,
  save_best_only = TRUE,
...
if (boot.num > 1)
  model <- load_model_weights_tf(model,
                                 filepath = paste0("intermediates/models/bGEV_linear_fit/boot_", 1))
history <- model %>% fit(
  list(X_boot, pred_u_boot, X_lonlat),
  Y_train,
  epochs = 200,
  shuffle = T,
  batch_size = 16,
...
model <- load_model_weights_tf(model,
                               filepath = paste0("intermediates/models/bGEV_linear_fit/boot_", boot.num))
...
st = "intermediates/predictions/bGEV_linear_fit"
dir.create(st)
save(
  pred_bGEV,
  pred_bGEV_boot,
  file = paste0(
    "intermediates/predictions/bGEV_linear_fit/boot_",
    boot.num,
    ".Rdata"
  )
)
...
st = "intermediates/scores/bGEV_linear"
dir.create(st)
save(nll,
     file = paste0(
       "intermediates/scores/bGEV_linear/test_loss_",
       boot.num,
       ".Rdata"
     ))
...
# Save model
model %>% save_model_tf(paste0("intermediates/models/bGEV_linear/boot_", boot.num))
...
print(sMAD)
save(sMAD,
     file = paste0("intermediates/scores/bGEV_linear/sMAD_", boot.num, ".Rdata"))
...
save(twcrps,
     file = paste0(
       "intermediates/scores/bGEV_linear/twCRPS_",
       boot.num,
       ".Rdata"
     ))
