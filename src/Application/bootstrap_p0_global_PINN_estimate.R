...
output <- pBranchjoined %>%
  ...
  model <- keras_model(
    inputs = c(input_additive,input_nn), 
    outputs = c(output)
  )
  summary(model)
 ...
 history <- model %>% fit(
    list(X_I_basis_boot,X_N_boot), Y_train,
    epochs = 1000, batch_size = 161,
    callback=list(checkpoint),
    validation_data=list(list(additive_input=X_I_basis_boot , nn_input=X_N_boot),Y_valid)
